...
A(BODY([T('Entre les deux, le soutien.', { bold: true }), T(' Une organisation ne fait pas que réclamer des retraits. Quand le ministre a pris une mesure qu’elle porte, la délégation le dit avant d’en venir au conflit, une fois par mesure et par organisation (deux fois par partie en moyenne). Et surtout, la règle qui manquait : ', {}), T('une organisation ne peut pas exiger le retrait d’une mesure qu’elle défend', { bold: true }), T('. Le jeu le vérifie désormais dans les porteurs de la carte, et il écarte aussi toute mesure qui fait monter l’adhésion des personnels — on ne demande pas le retrait de ce qui améliore le métier. N’avoir rien à contester est un résultat possible : un ministre qui joue le programme des organisations ne se voit jamais réclamer de retrait.', {})]));
A(BODY([T('Second temps : la revendication.', { bold: true }), T(' L’organisation exige le retrait de la mesure qu’elle conteste le plus, parmi celles qui restent après ce filtre. Trois réponses sont possibles. ', {}), T('Céder', { bold: true }), T(' retire vraiment la mesure du jeu : ses effets réels ne viendront jamais, et le compteur d’abandons monte. ', {}), T('Maintenir', { bold: true }), T(' face à un profil combatif, c’est provisionner une grève pour le printemps. ', {}), T('Requalifier', { bold: true }), T(' (la renommer et la rendre facultative) ne coûte presque rien sur le moment : le conflit se dénoue, les crédits restent inscrits, l’annonce survit. C’est le geste politique le plus fréquent du système français, et le seul dont le prix n’apparaît qu’au bilan : la mesure requalifiée produit moins d’un cinquième de ce qu’elle promettait. Précédent : le « choc des savoirs » de décembre 2023, requalifié en « groupes de besoins », puis vidé de son obligation.', {})]));
...
A(ENCADRE('À relire', 'Le rattachement des mesures aux organisations est lu dans la ligne « portée par » de chaque carte. Le document nº 2 en donne la liste complète. Y a-t-il des mesures que vous rattacheriez à une organisation, ou que vous en détacheriez ? Une erreur ici produit une scène fausse : une organisation qui réclame le retrait de ce qu’elle défend.', '3.3 · Qui porte quoi'));
...
A(BODY('Quand l’affaire sort à cause d’un mensonge à l’Élysée, l’écran le dit et cite la phrase : « à la question “Vos enfants sont-ils scolarisés dans le public ?”, vous aviez répondu : “Dans le public, bien sûr.” ». Sans ce rappel, le joueur voyait une affaire plus chère que les autres sans comprendre pourquoi ; avec, il relie la phrase à son prix, deux ans plus tard.'));
...
A(H2('4.2 La revanche : même partie, doctrine inversée'));
A(BODY('Au bilan, un second bouton : rejouer la même partie, doctrine inversée. La graine du hasard est conservée : mêmes tirages, mêmes crises, mêmes affaires. Seul le classement de juin 2027 est proposé à l’envers, et le joueur reste libre de le modifier. À la fin de cette seconde partie, le bilan affiche les deux résultats côte à côte : priorité déclarée, les trois scores, la durée du mandat, et les cinq compteurs réels.'));
A(BODY([T('C’est la thèse du jeu en une image : ', {}), T('les faits sont les mêmes, les mesures disponibles aussi ; ce qui a changé, ce sont les priorités, donc les choix, donc le pays à l’arrivée', { bold: true }), T('. Le désaccord politique porte là-dessus, pas sur les chiffres. En formation, c’est le moment de discussion collective le plus productif.', {})]));
A(VIDE());
A(ENCADRE('À relire', 'La revanche demande de rejouer une partie entière. Faut-il une version courte de la comparaison, jouée par le jeu lui-même (le même mandat rejoué automatiquement avec la doctrine inverse, en une seconde), pour ceux qui n’auront pas le temps ?', '4.2 · La revanche'));

A(H2('4.3 La boussole, de quels programmes vos mesures se rapprochent'));
